      ...
      {otp.map((data, index) => (
        <input
          ...
          style={{
            ...
            borderRadius: "12px", border: "2px solid #ddd", background: "#f8f9fa",
            fontWeight: "bold", color: "#1A3C34", outline: "none", transition: "border-color 0.2s",
          }}
        />
      ))}
    </div>
...
export const Login = () => {
  ...
  const googleLogin = useGoogleLogin({
    onSuccess: async (tokenResponse) => {
        // Warning: Google Auth route logic might need backend support (currently missing in auth.js)
      try {
        const res = await axios.post("http://localhost:5000/api/auth/google", {
          token: tokenResponse.access_token,
          role: role,
        });
        login({ ...res.data, userType: role });
        navigate(role === 'therapist' ? '/therapist-dashboard' : '/patient-dashboard');
      } catch (err) {
        setError("Google Login Failed - Backend support missing");
      }
    },
  ...
  const handleLogin = async (e) => {
    ...
    try {
      ...
      // We merge them so the AuthContext has all info
      const userData = {
        token: res.data.token,
        ...res.data.user,
        userType: role // ensure frontend knows the type
      };
  ...
  return (
    <div className="auth-container">
      <div className="auth-card">
        ...
        {error && (
          <div style={{ color: "#d32f2f", background: "#ffebee", padding: "10px", borderRadius: "8px", textAlign: "center", marginBottom: "15px", fontSize: "0.9rem" }}>
            {error}
          </div>
        )}
        ...
        <form onSubmit={handleLogin}>
          ...
          <button type="submit" className="auth-btn" disabled={isLoading} style={{ opacity: isLoading ? 0.7 : 1, cursor: isLoading ? "not-allowed" : "pointer" }}>
            {isLoading ? (
...
// --- SIGNUP COMPONENT ---
// NOTE: OTP routes (/send-otp) are missing in the backend. 
// For now, this is updated to point to port 5000, but may fail if backend routes aren't added.
export const Signup = () => {
  const navigate = useNavigate();
  const { login } = useAuth();
  const [role, setRole] = useState("patient");
  ...
  // Simplified Signup Fallback (Since backend only supports direct register)
  // If you want to use the OTP flow, you must implement /send-otp in backend.
  // This function tries the direct register route if OTP fails or as a default.
  ...
  const googleSignup = useGoogleLogin({
    onSuccess: async (tokenResponse) => {
      try {
        const res = await axios.post("http://localhost:5000/api/auth/google", {
          token: tokenResponse.access_token, role: role,
        });
        login({ ...res.data, userType: role });
        navigate("/");
      } catch (err) { setMessage("Google Signup Failed"); }
    },
  ...
  return (
    <div className="auth-container">
      <div className="auth-card">
        ...
        {message && (
          <div style={{ color: message.includes("created") ? "#2e7d32" : "#d32f2f", background: message.includes("created") ? "#e8f5e9" : "#ffebee", padding: "10px", borderRadius: "8px", textAlign: "center", marginBottom: "15px", fontSize: "0.9rem" }}>
            {message}
          </div>
        )}
        ...
        <form onSubmit={handleDirectSignup}>
            <div className="input-group">
              ...
              <input className="auth-input" type="text" placeholder="Full Name" required onChange={(e) => setFormData({ ...formData, name: e.target.value })} />
            </div>
            <div className="input-group">
              <Mail size={20} className="input-icon" />
              <input className="auth-input" type="email" placeholder="Email Address" required onChange={(e) => setFormData({ ...formData, email: e.target.value })} />
            ...
            {role === "therapist" && (
              <div className="input-group">
                ...
                <input className="auth-input" type="text" placeholder="Medical License ID" required onChange={(e) => setFormData({ ...formData, licenseId: e.target.value })} />
              </div>
            )}
            <div className="input-group">
              <Lock size={20} className="input-icon" />
              <input className="auth-input" type="password" placeholder="Create Password" required onChange={(e) => setFormData({ ...formData, password: e.target.value })} />
            </div>
